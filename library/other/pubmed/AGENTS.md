# PubMed Printed CLI Agent Guide

This directory is a generated `pubmed-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pubmed-pp-cli doctor --json
pubmed-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pubmed-pp-cli which "<capability>" --json
pubmed-pp-cli <command> --help
```
...
```bash
pubmed-pp-cli <command> --help
pubmed-pp-cli <command> --dry-run --agent
```
