...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pubmed-pp-cli",
		Short: `Pubmed CLI — Every PubMed E-utility, plus clinical-query refinement, a local article store, and trend/what's-new analytics no other…`,
		Long: `Pubmed CLI — Every PubMed E-utility, plus clinical-query refinement, a local article store, and trend/what's-new analytics no other PubMed tool ships.

Highlights (not in the official API docs):
  • clinical   Refine a clinical or disease topic to therapy, diagnosis, etiology, prognosis, clinical-prediction, or systematic-review evidence, with open-access, evidence-level, and human/language/age scope filte…
...
Health check: run 'pubmed-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pubmed-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pubmed-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	// Default to NCBI's public E-utilities limit (3 req/s). With an NCBI_API_KEY
	// the ceiling is 10 req/s — pass --rate-limit 10. Self-throttling here keeps
	// burst usage (loops, fan-out commands, agents) from tripping HTTP 429.
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 3, "Max requests per second (0 to disable; NCBI allows 3 without an API key, 10 with)")
	...
	rootCmd.AddCommand(newNovelAbstractCmd(flags))
	rootCmd.AddCommand(newNovelClinicalCmd(flags))
	rootCmd.AddCommand(newNovelCooccurCmd(flags))
	rootCmd.AddCommand(newNovelFindCmd(flags))
	rootCmd.AddCommand(newNovelPicoCmd(flags))
