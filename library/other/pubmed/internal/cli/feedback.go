...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PUBMED_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PUBMED_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pubmed-pp-cli/feedback")
...
When ` + "`PUBMED_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PUBMED_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pubmed-pp-cli feedback list
  pubmed-pp-cli feedback list --limit 5
  pubmed-pp-cli feedback list --json`,
